...
class Population:
    ...
    def speciate(self):
        ...
        for s in self.species:
            if s.shared_fitness < self.avg_fitness:
                s.stagnation += 1
            else:
                s.stagnation = 0
            if s.stagnation > 10:
    ...
    def createNextGen(self, pop_num):
        ...
        for s in self.species:
            if len(s.members) > 1:
                ...
                quarter = math.floor(len(fitness_sorted)/4)
                top_quarter_idx = (len(fitness_sorted) - 1) - quarter
                fittest_quarter = fitness_sorted[top_quarter_idx:]
                s.members = fittest_quarter
        ...
        #make sure best_genome has its fitness reset
        next_gen[0].fitness = 0
    ...
    def evolve(self, iterations):
        for i in range(iterations):
            ...
            for s in self.species:
                best_genome = max(s.members, key = lambda x : x.fitness)
                if i % 10 == 0:
                    snake.play(best_genome)
                best_fitness = best_genome.fitness
                if best_fitness > best:
                    best = best_fitness
                #best_genome.network.printAdjList()
                #best_genome.printGenome()
                print(str((len(s.members), (len(best_genome.nodeGenes), len(best_genome.connectionGenes)), round(best_genome.fitness, 3), s.stagnation, round(s.shared_fitness - self.avg_fitness, 3))) + ", ",end="")
                """
                avg_size_n = 0
                avg_size_c =0
                for g in s.members:
                ...
                """
            print(len(self.species), round(best, 3), round(self.avg_fitness, 3))
            #print("--------------")
